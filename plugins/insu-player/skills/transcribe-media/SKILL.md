...
Validate that VTT begins with "WEBVTT", contains cues, and has increasing timestamps. For proofreading, translation, or segmentation jobs, also validate that `transcript.json` contains non-empty normalized timed units and a BCP 47 source language when available. Report the provider, model, detected language, and timing granularity; never report or echo the API key. This skill transcribes speech but does not proofread, translate, or segment subtitles.
