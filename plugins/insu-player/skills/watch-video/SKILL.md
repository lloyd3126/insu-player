...
- Treat installed plugin cache directories such as `~/.codex/plugins/cache/` as immutable. Never patch cached skill files to recover a job. Report the current-contract defect, fix the canonical repository source, validate and release it, then update the installed plugin before retrying.
- If version 0.3.0 cleanup left a current-schema job at `failed / cleanup` immediately after `failed` or `interrupted / model_transcription`, run `scripts/repair-transcription-cleanup.sh WORKSPACE VIDEO_ID` once. It validates that exact history, refuses jobs with a current valid model transcript, and resets only the retry state. It is an operational repair, not a migration or legacy reader.
