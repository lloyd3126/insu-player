...
4. Port `8000` is only the preferred starting port, not a machine-wide library identity. Start normally without an explicit port. If `8000` is occupied, the server must atomically bind an OS-selected free localhost port and record the actual `host`, `port`, `pid`, `buildId`, and `statusSchemaVersion` in the selected workspace's `.insu-player-server.json`. Reuse a running server only when both its descriptor and `/api/health` report the exact current build ID and status schema. If that workspace is running another build, report its endpoint and explicitly stop that workspace-owned process before restarting. Never auto-stop or take over a different build. Leave every other workspace untouched. Treat an explicitly supplied port as strict.
...
- Read and write only the current contracts: job status schema 6, model transcript schema 2, proofread and translation manifest schema 5, segmentation manifest schema 4, and media catalog schema 1. Missing fields, older schemas, invalid timestamps, stale processor shapes, and unregistered paths fail closed. Do not add migrations, legacy readers, inferred defaults, filesystem-mtime substitutes, or synthetic failed rows. When old local data is disposable, remove it through the exact removal boundary and recreate current data.
