...
首頁是全高入口頁。navbar 依序顯示「開始說明」、「我的提示」、「轉錄設定」、「支援網站」、「擴充功能」與帶有 Library 圖示及項目數量的「影片中心」。「開始說明」以「1 初始化」、「2 加入影音」與「3 交給 Agent」三個 tabs 依序引導，每個 tab 只放一個主要段落。「我的提示」與「支援網站」是各自獨立、可由 URL 恢復的 dialog，內建使用情境提示位於「我的提示」建立卡下方。「擴充功能」會開啟標題為「Chrome 擴充功能」的獨立 dialog，以安裝、連接、使用三個 tabs 說明未封裝資料夾載入、從目前首頁直接連接精確 localhost origin、頁面／embed／MP4／HLS 加入，以及登入 Cookie 的單次本機使用邊界。

「轉錄設定」只顯示一張統一模型表格，欄位固定為選用、類型、模型、狀態與操作。模型欄取得剩餘寬度，其餘欄位依內容收合。詳情按鈕開啟可重新整理保留的 nested modal。本機詳情負責下載、取消、驗證與移除，雲端詳情負責該 provider 共用 Key 的設定與清除。「影片中心」在下載排程活躍時每秒更新，並在頂部以「我的影音」與「詳細資訊」tabs 切換。兩種檢視都直接顯示等待下載、下載中、需要確認、失敗與已完成影音，不另開下載頁，也不暴露批次。單一項目保留穩定 ID，下載完成時同一張卡片原位轉成可播放影音。「我的影音」放全寬搜尋列與最多三欄的卡片。「詳細資訊」收納摘要統計、狀態篩選、固定欄寬列表及單項重試或取消。全域佇列控制只顯示暫停或繼續、進行中、等待中與並行數。有項目時預設開啟前者，空庫時預設開啟後者。播放與詳情共用同一層 overlay，播放器在同源 iframe 內開啟。
