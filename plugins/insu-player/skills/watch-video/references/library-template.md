...
這個模板是 `insu-player` 流程的固定首頁。它不需前端建置工具，預設只使用本機字體與同源資產；使用者在設定中主動選擇 Google Fonts 時，才會向 Google 載入該字體。`library_server.py` 提供同源 JSON API、影片 Range request、縮圖、VTT、播放器頁面、播放進度端點，以及不落盤的工作階段環境變數端點。
