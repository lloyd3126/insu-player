...
usage() {
  printf 'usage: process-video.sh <workspace> <video-url> --language SOURCE_BCP47 [--translate TARGET_BCP47 | --proofread] [--device cpu|cuda] [--consent-to-audio-upload] [--allow-low-quality] [--no-transcribe]\n'
}
...
language_code=""; device_name="cpu"; consent_to_audio_upload=0; allow_low_quality=0; no_transcribe=0
...
if [ "$current_state" = "needs_transcription" ] && [ "$no_transcribe" -eq 0 ]; then
  ...
  if [ "$consent_to_audio_upload" -eq 1 ]; then transcribe_args+=(--consent-to-audio-upload); fi
