...
```bash
plugins/xeruca-player/skills/watch-video/scripts/doctor.sh <workspace>
```
...
這版透過 `imageio-ffmpeg` 的平台 wheel 取得真正的 FFmpeg binary，再複製到 `<workspace>/.agent-tools/xeruca-player/bin/ffmpeg`。不需要 `ffprobe`、Homebrew 或 `sudo`。
...
<workspace>/.agent-tools/xeruca-player/.venv/bin/yt-dlp --verbose --version
```
...
- 用 `doctor.sh` 檢查可用空間。
- 刪除前先確認模型目錄只屬於此 workflow：`<workspace>/.agent-tools/xeruca-player/models/`。
