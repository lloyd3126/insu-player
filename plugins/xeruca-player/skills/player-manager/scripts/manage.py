...
def latest_release() -> dict[str, Any]:
    request = urllib.request.Request(
        LATEST_RELEASE_API,
        headers={"Accept": "application/vnd.github+json", "User-Agent": "xeruca-player-updater"},
    )
...
def download(url: str, target: Path) -> None:
    request = urllib.request.Request(url, headers={"User-Agent": "xeruca-player-updater"})
...
def portable_update(
    ...
    *,
    ...
    local_root = root / ".local" / "player-manager"
