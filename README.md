...
5. Agent 先檢查目前環境並提出一個安全建議。使用者不需要選 skill、模型 ID、provider、processor、語言碼或字幕產物類型。內部仍會完整記錄語音時間、內容處理與字幕切分所用能力。
6. 工作超出目前 turn 時，由 `$monitor-player-job` 定期回到同一個 task，依 `status.json` 接續已授權階段。
7. 完成後在影音中心開啟播放，後續可從原進度續播。
...
- 預設建議：在本機用 Whisper medium 從原始音訊辨識語音，再由目前的 Agent 讀取轉錄文字，完成校正或翻譯與字幕切分。影音播放畫質不會為了加快語音辨識而降低。
- 全部留在本機：Agent 會先確認目前 workspace 的本機文字模型真的能完成所需語言的校正、翻譯與切分，不會把 Whisper 誤當翻譯模型。
- 使用 OpenAI API：只有實際需要時才會說明要上傳音訊、字幕文字或兩者、可能費用與本機替代方案。每一次上傳都需要使用者明確同意。
...
| 開始使用 | 貼上單支影音網址，確認格式正確後複製已帶入網址的完整提示 |
...
如果目前沒有需要保留的影音，或想用全新的資料驗證流程，可以請 Agent 完整重建目前專案的 INSU Player。這會清空影音、字幕、工作紀錄與 SQLite 資料庫，並移除舊的相容資料，但會保留程式碼、workspace-local Bun、Whisper 與已下載模型：

```text
完整重建目前專案的 INSU Player。清空影音、字幕、工作紀錄與資料庫，移除所有相容層；保留程式碼、Bun runtime、Whisper 與模型。請先產生唯讀預覽和 digest，等我確認後執行，最後重啟首頁並驗證影音庫為 0 筆。
```
...
程式修改完成後，還需通過八個 skill validator 與 plugin validator。介面圖示統一使用 [Lucide](https://lucide.dev/)，執行時不載入圖示 CDN。

長時間工作追蹤使用 ChatGPT／Codex 桌面版的 Scheduled tasks，並固定附著於目前 task、使用目前專案的 local checkout。它不會在 Hono、SQLite 或 `status.json` 中建立另一套 scheduler。需要本機影音、模型或 workspace 時，Mac 必須保持開機且 ChatGPT 桌面程式持續運作。沒有 Scheduled tasks 能力的環境會明確回報不支援，不提供舊式輪詢 fallback。
