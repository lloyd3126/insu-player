...
- 修正播放器 WebVTT cue block 解析，避免 cue identifier 與 NOTE 內容混入可見字幕
- 將原始、校正、翻譯與切分字幕合併為單一 catalog，並把多語言 artifact 依語言軌分列顯示與下載
- 強制所有字幕先校正再切分，只有需要其他語言時才從已驗證校正稿進行翻譯
- 移除字幕完成提示，只在尚無字幕且流程可開始時顯示「開始製作字幕」
- 統一狀態、畫質與字幕表格的 frame、表頭、資料列、標籤、操作欄與窄螢幕行為
...
- 待下載與下載中的影音直接出現在影片中心，統一以 SQLite operation 與 event 作為狀態事實來源，支援並行下載、暫停、繼續、取消、失敗重試與低畫質確認，不再顯示批次概念
