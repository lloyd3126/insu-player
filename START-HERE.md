...
> 請使用 $watch-video，第一個動作先啟動 INSU Player 首頁並用 Codex 內建瀏覽器開啟，保持首頁開啟後再執行環境檢查。所有工具、模型、cache、影片與字幕都必須留在這個資料夾。說明本機與 OpenAI API 轉錄的差異，並在取得字幕前問我要不要繁中翻譯，再把這支影片加入影片庫：VIDEO_URL
...
需要翻譯時使用 `--translate zh-TW --provider local|openai`：不取得任何平台字幕，改由選定模型轉錄原始音訊並產生英文詞級時間；初譯完成後重建完整句子並潤色，英文與繁中最後共用同一組句級時間段。若不需要翻譯，使用 `--no-translate`，此時才可沿用來源字幕。
