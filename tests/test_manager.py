...
SCRIPT = REPO_ROOT / "plugins" / "xeruca-player" / "skills" / "player-manager" / "scripts" / "manage.py"
...
class ManagerTests(unittest.TestCase):
    def test_current_checkout_is_detected(self) -> None:
        self.assertEqual(manager.plugin_version(), "0.1.0")
    ...
    def test_portable_update_preserves_local_data_and_removes_obsolete_managed_file(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            incoming = base / "xeruca-player-v0.2.0"
            incoming.mkdir()
