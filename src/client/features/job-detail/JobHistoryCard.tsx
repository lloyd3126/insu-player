...
export function JobHistoryCard({ history }: { history: JobHistoryEntry[] }) {
  return (
    <Table
      className="detail-table detail-table--compact history-table"
      containerClassName="detail-table-frame job-history-card history-table-container"
    >
      <TableHeader>
        <TableRow>
            ...
            <TableRow key={key}>
              ...
              <TableCell className="history-table__status">
                <Badge variant="outline">
                  {JOB_STATE_LABELS[entry.state ?? ""] ?? entry.state ?? "—"}
                </Badge>
              </TableCell>
